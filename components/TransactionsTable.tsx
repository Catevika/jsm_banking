  ...
  return (
    <Table>
      <TableHeader className='bg-[#f9fafb]'>
        <TableRow>
          <TableHead className="pl-2 pr-10px-2">Transaction</TableHead>
      ...
      <TableBody>
        {transactions.map((t: Transaction) => {
          ...
          return (
            <TableRow className={`${isDebit || amount[0] === '-' ? 'bg-[#FFFBFA]' : 'bg-[#F6FEF9]'} !hover:bg-none !border-b-default`} key={t.id}>
              <TableCell className="max-w-[250px] pl-2 pr-10">
                <div className='flex items-center gap-3'>
                  <h1 className='t-14 truncate font-semibold text[#344054]'>{removeSpecialCharacters(t.name)}</h1>
                </div>
              </TableCell>
